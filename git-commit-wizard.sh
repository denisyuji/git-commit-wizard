...
bullet_instruction=""
if $bullet; then
  bullet_instruction="- Format the commit description body as concise bullet points."
fi
...
readme_context=""
if [[ -n "$project_root" && -f "$project_root/README.md" ]]; then
  readme_context=$(head -n 200 "$project_root/README.md")
else
